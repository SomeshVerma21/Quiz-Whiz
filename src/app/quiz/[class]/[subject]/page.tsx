...
import type { Question, ClassLevel, SubjectName } from '@/lib/types';
...
export default function QuizPage() {
  ...
  const classVal = params.class ? parseInt(params.class as string) as ClassLevel : undefined;
  ...
  const loadQuizQuestions = useCallback(() => {
    ...
    if (classVal && subject) {
      let fetchedQuestions = getQuestions(classVal, subject); 

      if (fetchedQuestions.length === 0) {
        toast({
          title: "No Questions",
          description: `Sorry, no questions found for Class ${classVal} in ${subject}. Please try other options.`,
          variant: "destructive",
        });
        router.push('/'); 
        setIsLoading(false);
      ...
      setQuestions(questionsWithShuffledOptions);
       ...
       toast({
          ...
          description: "Could not determine class or subject.",
          variant: "destructive",
        });
      router.push('/');
      setIsLoading(false);
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[calc(100vh-4rem-1px)] p-4 text-center">
        ...
        <p className="text-md text-muted-foreground mb-6">
          We couldn't find any questions for the selected class and subject.
        </p>
